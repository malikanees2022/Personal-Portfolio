...
import { ThemeContext } from 'Y/app/context/themeContext';
  ...
  return (
    <nav className={styles.nav}>
      <div className={`container ${styles.navContainer}`}>
        <Link href={"/"} className={styles.navLogo} onClick={closeNavHandler}>Anees.</Link>
        {navOpen &&<ul className={styles.navItems}>
          {
            data.map(({id,link,caption})=>(
            <li key={id}>
              <Link href={link} onClick={closeNavHandler} 
